...
import '../../interfaces/IWStakingDualRewards.sol';

contract QuickDualSpell is WhitelistSpell {
  ...
  function addLiquidityWStakingRewards(
    ...
    address lp = getAndApprovePair(tokenA, tokenB);
    address rewardA = IWStakingDualRewards(wstaking).rewardA();
    address rewardB = IWStakingDualRewards(wstaking).rewardB();
    ...
    if (collSize > 0) {
      require(IWStakingDualRewards(collToken).getUnderlyingToken(collId) == lp, 'incorrect underlying');
      require(collToken == wstaking, 'collateral token & wstaking mismatched');
      bank.takeCollateral(wstaking, collId, collSize);
      IWStakingDualRewards(wstaking).burn(collId, collSize);
    }

    // 7. Put collateral
    ensureApprove(lp, wstaking);
    uint amount = IERC20(lp).balanceOf(address(this));
    uint id = IWStakingDualRewards(wstaking).mint(amount);
    if (!IWStakingDualRewards(wstaking).isApprovedForAll(address(this), address(bank))) {
      IWStakingDualRewards(wstaking).setApprovalForAll(address(bank), true);
    }
  ...
  function removeLiquidityWStakingRewards(
    ...
    IWStakingDualRewards(wstaking).burn(collId, amt.amtLPTake);
  ...
  function harvestWStakingRewards(address wstaking) external {
    ...
    address rewardB = IWStakingDualRewards(wstaking).rewardB();
    (, address collToken, uint collId, ) = bank.getCurrentPositionInfo();
    address lp = IWStakingDualRewards(wstaking).getUnderlyingToken(collId);
    require(whitelistedLpTokens[lp], 'lp token not whitelisted');
    require(collToken == wstaking, 'collateral token & wstaking mismatched');

    // 1. Take out collateral
    bank.takeCollateral(wstaking, collId, uint(-1));
    IWStakingDualRewards(wstaking).burn(collId, uint(-1));
